...
= 2. GPIO en el Microcontrolador K64
...
El *Bit-Banding* es un mecanismo de los núcleos Cortex-M3/M4 que permite modificar bits individuales de forma *atómica* [10].

== Funcionamiento y Mapeo
Este mecanismo mapea cada bit de una región de memoria (1 MB) a una palabra completa (32 bits) en una región llamada *Alias Region* (32 MB) [10]:
- Escribir el bit menos significativo en una palabra del *Alias* modifica directamente el bit correspondiente en la región de *Bit-band* [10].
- Esto elimina la necesidad de realizar operaciones de tipo *Lectura-Modificación-Escritura (RMW)*, evitando conflictos cuando ocurren interrupciones en medio de la operación [10].

== Regiones en K64F
Existen dos zonas principales habilitadas para bit-banding en el K64F [10]:
+ *SRAM_U:* Región de memoria RAM.
+ *Peripherals:* Región donde se encuentran los registros de los periféricos y GPIO [10].

#box(stroke: 1pt, inset: 10pt)[
  *Importante:* Para los puertos GPIO del K64, aunque existe la región de Bit-Band Alias (0x4200_0000), el hardware ya provee registros atómicos como PSOR y PCOR para manipular bits de salida de manera segura , [10].
]